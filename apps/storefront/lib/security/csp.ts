 ...
 * (cdn.razorpay.com, lumberjack.razorpay.com, payments.cashfree.com)
...
export function buildCsp(nonce: string, env = process.env.NODE_ENV): string {
  ...
  return [
    ...
    // (e.g. one-tap on storefront) is unblocked. Razorpay and Cashfree
    ...
    `script-src 'self' 'nonce-${nonce}'${devEval} https://accounts.google.com/gsi/client https://*.razorpay.com https://*.cashfree.com https://analytics.tesserix.app`,
    ...
    // leaves the modal blank. Cashfree's redirectTarget:"_modal"
    // checkout does the same from payments.cashfree.com.
