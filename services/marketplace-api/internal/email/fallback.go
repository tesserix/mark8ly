...
// FallbackSender chains two Senders: every Message goes to primary first,
// and only when primary errors does it retry on fallback. Production
// wiring is SendGrid (primary) → Resend (fallback) so a SendGrid outage,
// rate limit or account suspension degrades to a provider switch instead
// of dropped transactional mail.
//
...
// immediately — a malformed Message fails on every provider, so retrying
// it would only burn the fallback's quota.
func (f *FallbackSender) Send(ctx context.Context, msg Message) error {
	...
	f.log.Warn("email: primary provider failed, retrying on fallback",
	...
	if fallbackErr == nil {
		f.log.Info("email: fallback provider delivered",
			slog.String("to", msg.To),
			slog.String("subject", msg.Subject),
		)
		return nil
