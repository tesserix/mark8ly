// Package audit is platform-api's lightweight HTTP client for posting
// audit events to marketplace-api's /internal/audit-events ingest
// endpoint.
//
...
// never break business flows like invitation creation.
package audit
...
// store_id is optional — marketplace-api falls back to the tenant's
// first active store when omitted.
type Event struct {
...
// Emit posts a single event. Synchronous; callers on request paths
// should use EmitAsync instead. Failures are logged, never returned.
