...
// TestNewAccountService_OperatorPurgeSurvivesAbsentProviderAndFGA is the
// regression test for the typed-nil hazard newAccountService exists to
// close.
//
// platform-api deployed without ZITADEL_ENABLED has no account-deletion
// provider at all. A nil CONCRETE client handed straight to
// account.NewService becomes a NON-NIL interface holding a nil pointer: cleanupAfterTeardown's `if s.gip != nil` passes,
...
// return normally. MUTATION: pass a nil concrete client straight through
// in newAccountService instead of via the guarded interface variable and
// this test panics.
func TestNewAccountService_OperatorPurgeSurvivesAbsentProviderAndFGA(t *testing.T) {
	...
	// nil conn, nil fga, nil deleter — precisely the unconfigured
