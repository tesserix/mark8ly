...
8. **Document number sequencing is atomic, not lock-held-across-tx.** Both order numbers and return numbers follow `M-{store_prefix}-{yymmdd}-{seq}` / `R-{store_prefix}-{yymmdd}-{seq}`. A single `document_number_seq` table keyed on `(store_id, kind, day)` issues them via a single-statement atomic upsert (`INSERT ... ON CONFLICT (store_id, kind, day) DO UPDATE SET last_seq = document_number_seq.last_seq + 1 RETURNING last_seq`). This runs as its own statement inside the create transaction, holds the row lock for microseconds, and does **not** wrap the full multi-insert order flow. Rationale: the legacy `SELECT ... FOR UPDATE` pattern would serialize every concurrent checkout for a store on a single row across 4+ inserts — a latency cliff under burst on db-f1-micro. `kind IN ('order','return')` keeps the mechanism single-purpose and reusable.
