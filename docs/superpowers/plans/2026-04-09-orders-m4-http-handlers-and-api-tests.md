...
**Note on idempotency storage:** the M2 refund path uses an inline dedup via `order_events.payload->>'idempotency_key'` (see Orders M2 plan Task 10). It does **not** use the shared `idempotency_keys` table that products M2 ships — that table stores full response bodies for generic request replay, which is overkill for a single domain-bound action. If a future milestone introduces a generic "retry any POST with Idempotency-Key" envelope, the shared table is the right place; for slice 1, the inline dedup on `order_events` is enough.
