...
### KNOWN LIMITATION — A Verified-Email Match on an Existing Account Is Refused, Not Linked

**The design:** `idp/finish`'s registration path (a first-time Google sign-in whose intent resolves to no existing Zitadel user) first checks whether a Zitadel user already exists with that exact, verified email (`Client.FindUserByVerifiedEmail`). When one does, the right long-term outcome is to link the new Google identity onto that EXISTING account — otherwise one person ends up as two disconnected Zitadel identities, one password-based and one Google-based.

**What ships instead:** this handler refuses (`409 {"error":"account_exists_link_required"}`) rather than either creating a duplicate or performing that link. `Client.CreateHumanUserWithIDPLink`'s `idpLinks` parameter only works at creation time (`POST /v2/users/human`); attaching an IDP link to an ALREADY-EXISTING arbitrary human user is a separate v2 API surface this package does not call, because its exact shape was not confirmed against a live instance while this was written — guessing wrong here does not fail loudly, it silently mislinks or no-ops a real user's account, which is worse than a merchant seeing a clear "an account with this email already exists" message and using their password once. The refusal is intentionally visible rather than papered over.

**Also unconfirmed against a live instance:** `Client.FindUserByVerifiedEmail`'s `POST /v2/users` (ListUsers) request/response shape and `Client.CreateHumanUserWithIDPLink`'s `POST /v2/users/human` (AddHumanUser) shape are both modelled on Zitadel's documented v2 `UserService` proto, following this package's existing "observed vs documented" convention (see `client.go`'s package comment) — the retrieve/session-create/finalize calls elsewhere in this file WERE observed live; these two were not. Anyone shipping this to production should exercise both against the real TESSERIX org's Zitadel instance before relying on them, in particular the exact field name Zitadel uses for "email is verified" on a search result (`isVerified` here, by analogy with the human-user creation body) and the exact query message name for an equality email filter (`emailQuery` / `TEXT_QUERY_METHOD_EQUALS` here).
