...
var mobileOnlyRoutes = map[string]string{
	...
	"POST /auth/otp/resend":  "mobile email-OTP resend (#686 item 3): mails a fresh code AND returns a fresh sealed pending token, because the code and the challenge expire together. Unauthenticated like /auth/otp/verify above, for the same reason. The web admin resends at auth-bff's cookie-based /auth/otp/resend, which resumes from the pending cookie a native client does not have",
	"POST /auth/totp/verify": "mobile TOTP completion (#686 item 2): the authenticator-app half of obtaining a bearer token, so it is unauthenticated like /auth/otp/verify above. The web admin completes the same challenge at auth-bff's cookie-based /auth/zitadel/totp, which its Next.js server calls directly with the internal-auth secret a device cannot hold. Mobile needs its own route because that one requires an auth_request_id and workspace_tenant a device can never supply",
