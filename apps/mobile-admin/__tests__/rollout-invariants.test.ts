...
describe("Sweep A guard — a useCallback that calls a mutation's .mutate must depend on .mutate, not the mutation object", () => {
  ...
  // SAME pre-existing defect in eleven further files that predate increment
  // 3 (campaigns/coupons/segments/tickets detail+create screens, branding,
  // team invite) — out of this task's stated scope and recorded in the
  // report as a follow-up, not fixed here. The DETECTION logic below is
