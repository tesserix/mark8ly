...
// FallbackSender chains any number of Senders: every Message goes to the
...
// are returned immediately — a malformed Message fails on every provider,
...
func (f *FallbackSender) Send(ctx context.Context, msg Message) error {
	...
	for i, p := range f.providers {
		...
		if i < len(f.providers)-1 {
			f.log.Warn("email: provider failed, retrying on next",
				slog.String("provider", nameOf(p)),
