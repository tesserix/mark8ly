...
//
// The client gathers GIP credentials (via signInWithPassword on the
// mp-customer tenant), then hands the id_token + uid here. We resolve
// the store's tenant id from the slug, call auth-bff /auth/auto-login,
// and forward the Set-Cookie to the browser.
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        message:
          body?.message ?? "Sign-in failed. Please check your credentials.",
      };
    }

    // Forward the session cookie from auth-bff to the browser.
