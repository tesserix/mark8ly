...
//     in cmd/marketplace-api/main.go.
//
...
// NewFromConfig builds the process-wide Sender. Called exactly once in
// main so log-vs-real selection lives in one place instead of being
// re-decided per mailer.
//
// keys maps provider name → API key (empty = not configured). order is
// the EMAIL_PRIMARY_PROVIDER value — a single provider name or a
// comma-separated preference list ("resend" or "resend,sendgrid").
// Providers named there send first, in that order; configured providers
// not named follow in defaultOrder. Unknown names are warned about and
// skipped, so a typo can never take mail down.
//
...
//   - none configured         → LogSender (dev / CI — no emails sent)
//
// Every mailer renders before this layer, so emails are identical on all
...
func NewFromConfig(keys map[string]string, order string, log *slog.Logger) Sender {
	...
	case 1:
		log.Warn("email: single provider configured — no fallback",
			slog.String("provider", names[0]))
		return chain[0]
	default:
		log.Info("email: provider chain configured",
...
func resolveOrder(order string, log *slog.Logger) []string {
	...
	for _, raw := range strings.Split(order, ",") {
		...
		if _, ok := providerFactories[name]; !ok {
			log.Warn("email: unknown provider in EMAIL_PRIMARY_PROVIDER — skipping",
