...
**2. The nil-`*Emitter` contract is real and COMPLETE.** **Correction (fix-wave review, 2026-08-28): the package has eleven exported methods, not six** — enumerated by grepping every `func (e *Emitter)` across `internal/audit/*.go`, not just `emitter.go`. `Emit` (`:100`), `EmitSync` (`:146`) and `Stop` (`:173`) guard the receiver explicitly; `EmitStateTransition` (`:394`), `EmitAPIKeyEvent` (`:451`), `EmitPlanChange` (`:502`), `EmitCredentialAccess` (`credential_events.go:31`), `EmitPromoApplied` (`promo_events.go:23`), `EmitPromoCancelled` (`promo_events.go:59`), `EmitRefundIssued` (`refund_events.go:22`) and `EmitBillingArchived` (`refund_events.go:57`) build a map and then delegate to `e.Emit(...)`, which is legal on a nil receiver because they never dereference `e` themselves. So `var em *audit.Emitter` is a fully supported "auditing off". `internal/audit/nil_repo_test.go`'s `TestNilEmitter_AllExportedMethodsAreSafe` now pins all eleven, not just the original six.
...
- [x] Unit test: a nil `*Emitter` remains safe across **all eleven** exported methods — `Emit`, `EmitSync`, `Stop`, `EmitStateTransition`, `EmitAPIKeyEvent`, `EmitPlanChange`, `EmitCredentialAccess`, `EmitPromoApplied`, `EmitPromoCancelled`, `EmitRefundIssued`, `EmitBillingArchived`. This is the contract the fix now depends on. (Corrected during the fix-wave review, 2026-08-28: the original count of six missed the five methods defined outside `emitter.go`; see `internal/audit/nil_repo_test.go`'s `TestNilEmitter_AllExportedMethodsAreSafe`.)
